...
void ModuleObject::Initialize(char *aslr_base, Elf_Dyn *dynamic) {
    ...
    for (; dynamic->d_tag != DT_NULL; dynamic++) {
        switch (dynamic->d_tag) {
            ...
            case DT_RELAENT: {
                assert_(dynamic->d_un.d_val != sizeof(Elf_Rela));
                break;
            }

            case DT_SYMENT: {
                assert_(dynamic->d_un.d_val == sizeof(Elf_Sym));
                break;
            }
            ...
            case DT_RELENT: {
                assert_(dynamic->d_un.d_val == sizeof(Elf_Rel));
                break;
            }
            ...
            case DT_PLTREL: {
                ...
                assert_(value == DT_REL || value == DT_RELA);
                break;
            }
...
void ModuleObject::ResolveSymbolRelJumpSlot(Elf_Rel *entry,
    ...
    if (r_type == ARCH_JUMP_SLOT) {
        ...
        if (this->got_stub_ptr) {
            assert_(this->got_stub_ptr == (void*) target_address);
        } else {
